...
//	private $cacheActive = true;
	...
	{
        $modelPath = dirname(dirname(__DIR__)).'\\'.APPLICATION_PATH.'\\'.$type ?? $this->type.'\\'.$modelName;
...
//	}
	...
	public function render(string $viewName, $cacheName = '')
	{
        ...
        $this->params;
        $this->htmlDocument;
        require dirname(dirname(__DIR__)).DIRECTORY_SEPARATOR.APPLICATION_PATH.$this->type.'/View/'.$viewName.'.php';
        $includedContent = ob_get_contents();
        ob_end_clean();
        ob_start();
        require_once dirname(dirname(__DIR__)).DIRECTORY_SEPARATOR.APPLICATION_PATH.$this->type.'/View/'.$this->defaultView.'.php';
